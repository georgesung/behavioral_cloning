...
* drive.py - The script load the trained model, interface with the simulator, and drive the car
...
For normal driving data, I drove 3 laps the track, in the default direction (counter-clockwise looking from the top).
...
* Normal driving data
  ...
  * For the left/right camera images, I added an angle offset of +/- 0.15 (TODO: confirm) to the steering angle
...
Since driving data was collected using keyboard inputs, the input time-series is very choppy. To smooth the data, I used a simple moving average, with a window of 3 time units (i.e. 0.3 seconds). Below is a plot of the raw and smoothed steering angle vs. time, for a random section in my normal driving data:
...
In this project, I chose to use comma.ai's model, and train it from scratch using the data I've collected. The comma.ai model is a simple model, and its source code is available on Github. I initially experimented with a transfer learning approach using an ImageNet pre-trianed VGG16 base model, but I have yet to achieve good results with this approach.

The CNN model I used consists of the following layers:
...
Since it is impractical to store all images in memory, a python generator was used to generate batches of data. Thus, only a list of filenames of the entire training and validation set were stored in memory, and the images themselves were read from disk only to populate a new batch.

An adam optimizer (https://arxiv.org/abs/1412.6980v8) was used to minimize the mean squared error. The optimizer's learning rate was not extensively tuned, but I found a learning rate of 1e-4 produced stable results.

For regularization, the comma.ai model already specified dropout layers and their associated drop probabilities, and I used those values as-is. The training loss was always close to the validation loss after each epoch, so no signs of overfitting were seen. I attempted to add L2 regularization to the model, but empirical results on the test track were worse, so I excluded L2 regularization in the final model.
...
<TODO: say something about the plot?>
